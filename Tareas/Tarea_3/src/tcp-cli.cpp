...
int main( int argc, char * argv[] ) {
   const char * legov6 = "fe80::8f5a:e2e1:7256:ffe3%enp0s31f6";
   const char * legov4 = "10.1.35.1";
   const char * request = "GET /lego/index.php?figure=elephant&part=1 HTTP/1.1\r\nhost: redes.ecci\r\n\r\n";
   VSocket * client;
   char a[512];
   ...
   if ( ssl ) {
      ...
      if ( 4 == ipVer ) {
         ...
         client->Connect( "os.ecci.ucr.ac.cr", "https" );
         printf( " IPv4\n" );
      } else {
         client = new SSLSocket( true );	// Create an IPv6 TCP SSL socket
         client->Connect( legov6, "https" );
         printf( " IPv6\n" );
      }
   } else {							// Non SSL socket
      ...
      if ( 4 == ipVer ) {
         ...
         client->Connect( legov4, 80 );
         printf( " IPv4\n" );
      } else {
         client = new Socket( 's', true );	// Create an IPv6 TCP socket
         client->Connect( legov6, "http" );
         printf( " IPv6\n" );
      }
   }
