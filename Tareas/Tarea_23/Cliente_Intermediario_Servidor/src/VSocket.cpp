...
int VSocket::Bind(int port) {
   ...
   if (!this->IPv6) {
      ...
      st = bind(this->idSocket, (struct sockaddr*)&my_addr, sizeof(my_addr));
      ...
      st = bind(this->idSocket, (struct sockaddr*)&my_addr6, sizeof(my_addr6));
...
void VSocket::EnableBroadcast() {
   int broadcast = 1;

   int st = setsockopt(this->idSocket, SOL_SOCKET, SO_BROADCAST,
                       &broadcast, sizeof(broadcast));

   if (st == -1) {
      throw std::runtime_error("ERROR EnableBroadcast()");
   }
}

size_t VSocket::Broadcast(const void* buffer, size_t size,
   ...
   int st = inet_pton(AF_INET, broadcastIp, &addr.sin_addr);

   if (st <= 0) {
      throw std::runtime_error("ERROR Broadcast() inet_pton");
   }

   return this->sendTo(buffer, size, &addr);
}
