...
int main () {
   ...
   while (seguir) {
     ...
     if (op == 1) {
         ...
         std::string anterior1 = fs.getPiezas(nombre, 1);
         std::string anterior2 = fs.getPiezas(nombre, 2);

         std::string nuevoContenido;

         if (!anterior1.empty()) {
            nuevoContenido += "1\n" + anterior1;
            if (nuevoContenido[nuevoContenido.length() - 1] != '\n') {
               nuevoContenido += "\n";
            }
         }
         ...
         while (true) {
            ...
            if (!pieza.empty()) {
               nuevoContenido += pieza + "\n";
            }
         }

         fs.crearFigura(nombre, nuevoContenido);
